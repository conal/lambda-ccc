...
import Language.HERMIT.Primitive.GHC (rule,coreExprFreeIds)
...
applyRules :: [String] -> RewriteH CoreExpr
applyRules nms = catchesM (rule <$> nms)
...
reifyExpr =
  do varId#    <- findIdT 'E.var#
     ...
     let rew :: ReExpr
         ...
         -- TODO: consider the second value generated by caseT, namely the wild
         -- variable. It's sometimes irrelevant, but not always!
         -- 
              ...
              altT idR (const idR) rew $ \ _ _ rhs' ->
                let pat  = apps pairPatId (varType <$> vars) vPats
                    pat' | wild `S.member` coreExprFreeIds rhs'
                         = apps asPatId# [varType wild] [varLitE wild]
                         | otherwise = pat
                in
                  (pat', rhs')
